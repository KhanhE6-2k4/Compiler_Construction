...
void compileCallSt(void) {
  ...
  if (isPredefinedProcedure(proc)) {
    ...
    compileArguments(proc->procAttrs->paramList);
    genCALL(getScopeLevel(symtab->currentScope) - getScopeLevel(proc->procAttrs->scope),
...
void compileIfSt(void) {
  ...
  if (lookAhead->tokenType == KW_ELSE) {
    ...
    // Update false jump to else block
    ...
    // No else, just update false jump to end
    updateFJ(fjLabel, getCurrentCodeAddress());
  }
}
...
void compileWhileSt(void) {
  ...
  // Update false jump to end of loop
  updateFJ(fjLabel, getCurrentCodeAddress());
}
...
void compileForSt(void) {
  ...
  // Check and get the loop variable
  var = checkDeclaredLValueIdent(currentToken->string);
  
  // Generate address for assignment and keep it on stack
  if (var->kind == OBJ_VARIABLE)
    genVariableAddress(var);
  
  // Copy address for assignment
  genCV();
  ...
  // Store initial value (address still on stack after ST)
  genST();
  
  // Copy address for loading value
  genCV();
  
  // Load value indirectly
  genLI();
  
  // Save the limit value on stack
  eat(KW_TO);
  
  // Loop start - load limit and compare
  loopStart = getCurrentCodeAddress();
  ...
  // Update false jump to exit
  updateFJ(fjLabel, getCurrentCodeAddress());
  
  // Clean up address from stack
  genDCT(1);
}
...
Type* compileFactor(void) {
  ...
  case TK_CHAR:
    ...
    genLC((WORD)currentToken->string[0]);
    type = charType;
    break;
  case TK_IDENT:
    eat(TK_IDENT);
	...
	compileArguments(obj->funcAttrs->paramList);
	genCALL(getScopeLevel(symtab->currentScope) - getScopeLevel(obj->funcAttrs->scope),
